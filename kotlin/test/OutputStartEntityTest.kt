...
class OutputStartEntityTest {
  ...
  @Test
  fun basic() {
    val setup = outputStartBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_start.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_START_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val outputStartRef01Ent = client.outputStart(null)
    var outputStartRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.output_start"), "output_start_ref01")) ?: linkedMapOf())

    val outputStartRef01DataResult = outputStartRef01Ent.create(outputStartRef01Data, null)
    outputStartRef01Data = Helpers.toMapAny(if (outputStartRef01DataResult is SdkEntity) outputStartRef01DataResult.data() else outputStartRef01DataResult) ?: linkedMapOf()
    assertNotNull(outputStartRef01Data, "expected create result to be a map")
    assertNotNull(outputStartRef01Data["id"], "expected created entity to have an id")

  }

  companion object {
    fun outputStartBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_start test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_START_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_START_ENTID"] = idmap
