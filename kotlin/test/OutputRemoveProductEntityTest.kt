...
class OutputRemoveProductEntityTest {

  @Test
  fun instance() {
    val testsdk = BluefinTecsMerchantPortalSDK.testSDK()
    val ent = testsdk.outputRemoveProduct(null)
    assertNotNull(ent, "expected non-null output_remove_product entity")
  }

  @Test
  fun basic() {
    val setup = outputRemoveProductBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_remove_product.$op", mode)
    ...
    var outputRemoveProductRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.output_remove_product"), "output_remove_product_ref01")) ?: linkedMapOf())

    val outputRemoveProductRef01DataResult = outputRemoveProductRef01Ent.create(outputRemoveProductRef01Data, null)
    outputRemoveProductRef01Data = Helpers.toMapAny(if (outputRemoveProductRef01DataResult is SdkEntity) outputRemoveProductRef01DataResult.data() else outputRemoveProductRef01DataResult) ?: linkedMapOf()
    assertNotNull(outputRemoveProductRef01Data, "expected create result to be a map")

  }

  companion object {
    fun outputRemoveProductBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_remove_product test data: " + e.message, e)
      }
      ...
      idnames.add("output_remove_product01")
      idnames.add("output_remove_product02")
      idnames.add("output_remove_product03")
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_REMOVE_PRODUCT_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_REMOVE_PRODUCT_ENTID"] = idmap
