...
class OutputMessageDirectTest {

  @Test
  fun directLoadOutputMessage() {
    val setup = directSetup(jm("id", "direct01"))
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-load-output_message", mode)
  ...
  companion object {
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_MESSAGE_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_MESSAGE_ENTID"]
