...
class MerchantPortalPamContractControllerEntityTest {
  ...
  @Test
  fun basic() {
    val setup = merchantPortalPamContractControllerBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "merchant_portal_pam_contract_controller.$op", mode)
    ...
    var merchantPortalPamContractControllerRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
  ...
  companion object {
    fun merchantPortalPamContractControllerBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read merchant_portal_pam_contract_controller test data: " + e.message, e)
      }
