...
class OutputStatusDirectTest {

  @Test
  fun directLoadOutputStatus() {
    val setup = directSetup(jm("id", "direct01"))
    val mode = if (setup.live) "live" else "unit"
    val reason = RunnerSupport.skipReason("direct", "direct-load-output_status", mode)
  ...
  companion object {
    ...
    class DirectSetup {
      lateinit var client: BluefinTecsMerchantPortalSDK
    ...
    fun directSetup(mockres: Any?): DirectSetup {
      ...
      envm["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_STATUS_ENTID"] = linkedMapOf<String, Any?>()
      ...
      if (live) {
        ...
        setup.client = BluefinTecsMerchantPortalSDK(mergedOpts)
        setup.live = true

        var idmap: MutableMap<String, Any?> = linkedMapOf()
        val entidRaw = env["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_STATUS_ENTID"]
      ...
      setup.client = BluefinTecsMerchantPortalSDK(jm(
