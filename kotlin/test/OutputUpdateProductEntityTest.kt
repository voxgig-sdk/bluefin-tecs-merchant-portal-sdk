...
class OutputUpdateProductEntityTest {
  ...
  @Test
  fun basic() {
    val setup = outputUpdateProductBasicSetup(null)
    // Per-op sdk-test-control.json skip.
    val mode = if (setup.live) "live" else "unit"
    for (op in arrayOf("create")) {
      val reason = RunnerSupport.skipReason("entityOp", "output_update_product.$op", mode)
    ...
    Assumptions.assumeFalse(
      setup.syntheticOnly,
      "live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_UPDATE_PRODUCT_ENTID JSON to run live",
    )
    val client = setup.client

    // CREATE
    val outputUpdateProductRef01Ent = client.outputUpdateProduct(null)
    var outputUpdateProductRef01Data: MutableMap<String, Any?> = (Helpers.toMapAny(Struct.getprop(
        Struct.getpath(setup.data, "new.output_update_product"), "output_update_product_ref01")) ?: linkedMapOf())

    val outputUpdateProductRef01DataResult = outputUpdateProductRef01Ent.create(outputUpdateProductRef01Data, null)
    outputUpdateProductRef01Data = Helpers.toMapAny(if (outputUpdateProductRef01DataResult is SdkEntity) outputUpdateProductRef01DataResult.data() else outputUpdateProductRef01DataResult) ?: linkedMapOf()
    assertNotNull(outputUpdateProductRef01Data, "expected create result to be a map")

  }

  companion object {
    fun outputUpdateProductBasicSetup(extra: MutableMap<String, Any?>?): RunnerSupport.EntityTestSetup {
      ...
      try {
        ...
        throw AssertionError("failed to read output_update_product test data: " + e.message, e)
      }
      ...
      val entidEnvRaw = RunnerSupport.getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_UPDATE_PRODUCT_ENTID")
      val idmapOverridden = entidEnvRaw != null && entidEnvRaw.trim().startsWith("{")

      val envm = linkedMapOf<String, Any?>()
      envm["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_UPDATE_PRODUCT_ENTID"] = idmap
