// Generated direct-call tests for the merchant_portal_common_controller entity (unit mode;
...
static MerchantPortalCommonControllerDirectSetup merchant_portal_common_controller_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
  ...
  auto client = std::make_shared<BluefinTecsMerchantPortalSDK>(opts);

  MerchantPortalCommonControllerDirectSetup s;
  s.client = client;
  s.calls = calls;
  s.live = false;
  return s;
...
static void merchant_portal_common_controller_direct_load() {
  auto setup = merchant_portal_common_controller_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-merchant_portal_common_controller", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("merchantportalws/logDeveloperInfo")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(merchant_portal_common_controller_direct_load);
  return sdktest::summary("merchant_portal_common_controller_direct_test");
}
