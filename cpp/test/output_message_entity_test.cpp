...
static void output_message_entity_basic() {
  ...
  Value output_message_ref01_match_dt0 = vmap({{"id", getp(output_message_ref01_data, "id")}});
  Value output_message_ref01_data_dt0_loaded = output_message_ref01_ent->load(Struct::clone(output_message_ref01_match_dt0), Value::undef())->data();
  Value output_message_ref01_data_dt0_load_result = Helpers::toMapAny(output_message_ref01_data_dt0_loaded);
  ASSERT_TRUE(output_message_ref01_data_dt0_load_result.is_map(), "expected load result to be a map");
  ASSERT_EQ_VAL(getp(output_message_ref01_data_dt0_load_result, "id"), getp(output_message_ref01_data, "id"), "expected load result id to match");
