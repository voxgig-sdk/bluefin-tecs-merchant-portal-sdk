// Generated direct-call tests for the output_detail entity (unit mode;
...
static OutputDetailDirectSetup output_detail_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void output_detail_direct_load() {
  auto setup = output_detail_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-output_detail", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("merchantportalws/batch/registerAdditionalTerminal/details/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(output_detail_direct_load);
  return sdktest::summary("output_detail_direct_test");
}
