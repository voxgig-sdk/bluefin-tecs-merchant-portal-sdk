...
struct MerchantPortalPamMerchantControllerSetup {
...
static MerchantPortalPamMerchantControllerSetup merchant_portal_pam_merchant_controller_basic_setup(const Value& extra) {
  load_env_local();

  std::string entity_data_file = "../.sdk/test/entity/merchant_portal_pam_merchant_controller/MerchantPortalPamMerchantControllerTestData.json";
  ...
  Value idmap = Struct::transform(
      vlist({Value("merchant_portal_pam_merchant_controller01"), Value("merchant_portal_pam_merchant_controller02"), Value("merchant_portal_pam_merchant_controller03")}),
      vmap({{"`$PACK`", vlist({
        Value(""),
        vmap({
          {"`$KEY`", Value("`$COPY`")},
...
static void merchant_portal_pam_merchant_controller_entity_instance() {
  auto testsdk = BluefinTecsMerchantPortalSDK::testSDK();
  auto ent = testsdk->merchant_portal_pam_merchant_controller();
...
static void merchant_portal_pam_merchant_controller_entity_basic() {
  ...
  for (const std::string& op : std::vector<std::string>{"create"}) {
    auto sk = is_control_skipped("entityOp", std::string("merchant_portal_pam_merchant_controller.") + op, mode);
    if (sk.first) { std::cerr << "skip: " << (sk.second.empty()? "sdk-test-control.json" : sk.second) << "\n"; return; }
  }
  auto client = setup.client;
  // CREATE
  auto merchant_portal_pam_merchant_controller_ref01_ent = client->merchant_portal_pam_merchant_controller();
