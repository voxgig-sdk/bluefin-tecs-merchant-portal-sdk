// Generated direct-call tests for the output_message entity (unit mode;
...
static OutputMessageDirectSetup output_message_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void output_message_direct_load() {
  auto setup = output_message_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-output_message", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("merchantportalws/batch/registerAdditionalTerminal/restart/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(output_message_direct_load);
  return sdktest::summary("output_message_direct_test");
}
