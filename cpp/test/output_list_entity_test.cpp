...
static OutputListSetup output_list_basic_setup(const Value& extra) {
  ...
  Value idmap_resolved = Helpers::toMapAny(getp(env, "BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_LIST_ENTID"));
...
static void output_list_entity_basic() {
  ...
  {
    Value output_list_ref01_data_result = output_list_ref01_ent->create(Struct::clone(output_list_ref01_data), Value::undef())->data();
