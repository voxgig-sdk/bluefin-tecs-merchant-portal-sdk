...
static OutputMoveTidSetup output_move_tid_basic_setup(const Value& extra) {
  load_env_local();

  std::string entity_data_file = "../.sdk/test/entity/output_move_tid/OutputMoveTidTestData.json";
  ...
  Value idmap = Struct::transform(
      vlist({Value("output_move_tid01"), Value("output_move_tid02"), Value("output_move_tid03")}),
      vmap({{"`$PACK`", vlist({
        Value(""),
        vmap({
          {"`$KEY`", Value("`$COPY`")},
...
static void output_move_tid_entity_instance() {
  auto testsdk = BluefinTecsMerchantPortalSDK::testSDK();
  auto ent = testsdk->output_move_tid();
...
static void output_move_tid_entity_basic() {
  ...
  for (const std::string& op : std::vector<std::string>{"create"}) {
    auto sk = is_control_skipped("entityOp", std::string("output_move_tid.") + op, mode);
    if (sk.first) { std::cerr << "skip: " << (sk.second.empty()? "sdk-test-control.json" : sk.second) << "\n"; return; }
  }
  auto client = setup.client;
  // CREATE
  auto output_move_tid_ref01_ent = client->output_move_tid();
