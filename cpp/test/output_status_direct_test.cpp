// Generated direct-call tests for the output_status entity (unit mode;
...
static OutputStatusDirectSetup output_status_direct_setup(const Value& mockres) {
  Value calls = vlist();
  Value cshared = calls;
...
static void output_status_direct_load() {
  auto setup = output_status_direct_setup(vmap({{"id", Value("direct01")}}));
  auto sk = is_control_skipped("direct", "direct-load-output_status", "unit");
  ...
  Value result = client->direct(vmap({
    {"path", Value("merchantportalws/batch/registerAdditionalTerminal/status/{id}")},
    {"method", Value("GET")},
    {"params", params}
  }));
...
int main() {
  T_RUN(output_status_direct_load);
  return sdktest::summary("output_status_direct_test");
}
