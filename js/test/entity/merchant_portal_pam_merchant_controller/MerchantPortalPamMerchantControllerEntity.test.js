...
describe('MerchantPortalPamMerchantControllerEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        merchant_portal_pam_merchant_controller: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.MerchantPortalPamMerchantController().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.MerchantPortalPamMerchantController().stream('list')) {
