...
describe('OutputStatusEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        output_status: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.OutputStatus().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.OutputStatus().stream('list')) {
  ...
  test('basic', async () => {
    ...
    const output_status_ref01_data_dt0 = await output_status_ref01_ent.load(output_status_ref01_match_dt0)
    assert(null != output_status_ref01_data_dt0)
