...
describe('MerchantPortalPamPackageControllerEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        merchant_portal_pam_package_controller: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.MerchantPortalPamPackageController().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.MerchantPortalPamPackageController().stream('list')) {
