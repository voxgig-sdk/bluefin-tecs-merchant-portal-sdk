...
describe('OutputRemoveProductEntity', async () => {
  ...
  test('stream', async () => {
    const seed = {
      entity: {
        output_remove_product: { s1: { id: 's1' }, s2: { id: 's2' }, s3: { id: 's3' } }
      }
    }
    ...
    const seen = []
    for await (const item of base.OutputRemoveProduct().stream('list')) {
      seen.push(item)
    }
    ...
    if (config.feature && config.feature.streaming) {
      ...
      const got = []
      for await (const item of sdk.OutputRemoveProduct().stream('list')) {
