// BluefinTecsMerchantPortalSDK client (generated — mirrors the rust Main fragment).
...
BluefinTecsMerchantPortalSDK* bluefintecsmerchantportal_sdk_new(voxgig_value* options) {
  BluefinTecsMerchantPortalSDK* sdk = (BluefinTecsMerchantPortalSDK*)calloc(1, sizeof(BluefinTecsMerchantPortalSDK));
...
static bool sdk_op_allowed(BluefinTecsMerchantPortalSDK* sdk, const char* op) {
...
static voxgig_value* sdk_op_denied(BluefinTecsMerchantPortalSDK* sdk, const char* op) {
  voxgig_value* allow_op = getpath2(sdk->options, "allow", "op");
...
static voxgig_value* sdk_raw_request(
  BluefinTecsMerchantPortalSDK* sdk, voxgig_value* fetchargs, PNError** err) {
  *err = NULL;
  Utility* utility = sdk->utility;
...
voxgig_value* sdk_graphql(BluefinTecsMerchantPortalSDK* sdk, const char* query,
  ...
  *err = NULL;
  ...
  if (voxgig_is_list(errors) && 0 < voxgig_as_list(errors)->len) {
    ...
    char msg[512];
    snprintf(msg, sizeof(msg), "BluefinTecsMerchantPortalSDK: graphql: %s", m);
...
BluefinTecsMerchantPortalSDK* test_sdk(voxgig_value* testopts, voxgig_value* sdkopts) {
  ...
  BluefinTecsMerchantPortalSDK* sdk = bluefintecsmerchantportal_sdk_new(sdkopts);
  free(sdk->mode);
  sdk->mode = strdup("test");
  return sdk;
}
