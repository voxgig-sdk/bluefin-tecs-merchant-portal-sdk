// Generated direct-call test for the output_message entity (mirrors the
...
static voxgig_value* output_message_mock(void* ud, voxgig_value* args) {
  CALLS++;
...
static BluefinTecsMerchantPortalSDK* output_message_direct_setup(voxgig_value* mockres) {
  voxgig_value* opts = cmap(2,
    "base", v_str("http://localhost:8080"),
    "system", cmap(1, "fetch", vfn(output_message_mock, mockres)));
...
int main(void) {
  ...
  // LOAD
  {
    ...
    BluefinTecsMerchantPortalSDK* sdk = output_message_direct_setup(mockres);
    voxgig_value* params = v_map();
    setp(params, "id", v_str("direct01"));
    PNError* err = NULL;
    voxgig_value* result = sdk_direct(sdk, cmap(3,
      "path", v_str("merchantportalws/batch/registerAdditionalTerminal/restart/{id}"),
      "method", v_str("GET"),
      "params", params), &err);
  ...
  TEST_SUMMARY("bluefin_tecs_merchant_portal_output_message_direct");
}
