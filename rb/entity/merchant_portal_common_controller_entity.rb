...
class MerchantPortalCommonControllerEntity
  ...
  def make
    opts = @_entopts.dup
    MerchantPortalCommonControllerEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = BluefinTecsMerchantPortalHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [MerchantPortalCommonController, Hash] the current MerchantPortalCommonController data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = BluefinTecsMerchantPortalHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of MerchantPortalCommonController fields)
  def match_get
  ...
  #
  # @param reqmatch [MerchantPortalCommonControllerLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.MerchantPortalCommonController.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [MerchantPortalCommonController, Hash] the loaded MerchantPortalCommonController; raises BluefinTecsMerchantPortalError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = BluefinTecsMerchantPortalHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
