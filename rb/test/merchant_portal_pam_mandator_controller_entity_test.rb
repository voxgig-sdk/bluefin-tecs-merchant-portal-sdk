...
class MerchantPortalPamMandatorControllerEntityTest < Minitest::Test
  ...
  def test_basic_flow
    setup = merchant_portal_pam_mandator_controller_basic_setup(nil)
    # Per-op sdk-test-control.json skip.
    _live = setup[:live] || false
    ["create"].each do |_op|
      _should_skip, _reason = Runner.is_control_skipped("entityOp", "merchant_portal_pam_mandator_controller." + _op, _live ? "live" : "unit")
      if _should_skip
    ...
    # CREATE
    merchant_portal_pam_mandator_controller_ref01_ent = client.MerchantPortalPamMandatorController(nil)
...
def merchant_portal_pam_mandator_controller_basic_setup(extra)
  Runner.load_env_local

  entity_data_file = File.join(__dir__, "..", "..", ".sdk", "test", "entity", "merchant_portal_pam_mandator_controller", "MerchantPortalPamMandatorControllerTestData.json")
