...
class OutputMessageDirectTest < Minitest::Test
  def test_direct_load_output_message
    setup = output_message_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-output_message", setup[:live] ? "live" : "unit")
    if _should_skip
...
def output_message_direct_setup(mockres)
