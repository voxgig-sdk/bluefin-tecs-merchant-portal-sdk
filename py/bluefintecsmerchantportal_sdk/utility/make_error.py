...
def make_error_util(ctx, err):
    if ctx is None:
        from bluefintecsmerchantportal_sdk.core.context import BluefinTecsMerchantPortalContext
        ctx = BluefinTecsMerchantPortalContext({}, None)
    ...
    result = ctx.result
    if result is None:
        result = BluefinTecsMerchantPortalResult({})
    result.ok = False
    ...
    errmsg = ""
    if isinstance(err, BluefinTecsMerchantPortalError):
        errmsg = err.msg
    ...
    sdk_err = BluefinTecsMerchantPortalError("", msg, ctx)
