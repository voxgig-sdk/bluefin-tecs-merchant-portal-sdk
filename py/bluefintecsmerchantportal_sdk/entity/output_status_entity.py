...
class OutputStatusEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OutputStatusEntity(self._client, opts)
    ...
    def data_get(self) -> OutputStatus:
    ...
    def match_get(self) -> OutputStatus:
    ...
    def load(self, reqmatch=None, ctrl=None) -> OutputStatus:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OutputStatus().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
