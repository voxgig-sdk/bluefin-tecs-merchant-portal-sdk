...
class OutputMessageEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OutputMessageEntity(self._client, opts)
    ...
    def data_get(self) -> OutputMessage:
    ...
    def match_get(self) -> OutputMessage:
    ...
    def load(self, reqmatch=None, ctrl=None) -> OutputMessage:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OutputMessage().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
