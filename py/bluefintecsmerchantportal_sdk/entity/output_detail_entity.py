...
class OutputDetailEntity:
    ...
    def make(self):
        ...
        for k, v in self._entopts.items():
            opts[k] = v
        return OutputDetailEntity(self._client, opts)
    ...
    def data_get(self) -> OutputDetail:
    ...
    def match_get(self) -> OutputDetail:
    ...
    def load(self, reqmatch=None, ctrl=None) -> OutputDetail:
        utility = self._utility
        # reqmatch is optional: an entity with no id-like key loads with no
        # match. Treat None as an empty match so client.OutputDetail().load()
        # works with no args.
        if reqmatch is None:
            reqmatch = {}
