...
class TestOutputMessageDirect:

    def test_should_direct_load_output_message(self):
        setup = _output_message_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-output_message", "live" if setup["live"] else "unit")
        if _skip:
...
def _output_message_direct_setup(mockres):
