# BluefinTecsMerchantPortal SDK test runner
...
class BluefinTecsMerchantPortalTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            BluefinTecsMerchantPortalTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = BluefinTecsMerchantPortalTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = BluefinTecsMerchantPortalTestRunner.getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_LIVE")
        override = BluefinTecsMerchantPortalTestRunner.getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = BluefinTecsMerchantPortalTestRunner.getenv(key)
        ...
        explain = BluefinTecsMerchantPortalTestRunner.getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_EXPLAIN")
    ...
    def load_test_control():
        ...
        """
        if BluefinTecsMerchantPortalTestRunner._test_control is not None:
            return BluefinTecsMerchantPortalTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                BluefinTecsMerchantPortalTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            BluefinTecsMerchantPortalTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = BluefinTecsMerchantPortalTestRunner.load_test_control()
...
def env_override(m):
    return BluefinTecsMerchantPortalTestRunner.env_override(m)


def entity_data(v):
    return BluefinTecsMerchantPortalTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return BluefinTecsMerchantPortalTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return BluefinTecsMerchantPortalTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return BluefinTecsMerchantPortalTestRunner.load_test_control()


def live_client_options():
    return BluefinTecsMerchantPortalTestRunner.live_client_options()


def live_delay_ms():
    return BluefinTecsMerchantPortalTestRunner.live_delay_ms()
