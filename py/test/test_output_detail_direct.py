...
class TestOutputDetailDirect:

    def test_should_direct_load_output_detail(self):
        setup = _output_detail_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-output_detail", "live" if setup["live"] else "unit")
        if _skip:
...
def _output_detail_direct_setup(mockres):
