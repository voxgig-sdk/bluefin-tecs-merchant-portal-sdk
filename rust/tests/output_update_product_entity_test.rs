...
#[test]
fn output_update_product_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.output_update_product(Value::Noval);
    assert_eq!(ent.get_name(), "output_update_product");
}

#[test]
fn output_update_product_entity_stream() {
    ...
    let plainent = plainsdk.output_update_product(Value::Noval);
...
#[test]
fn output_update_product_entity_basic() {
    let setup = output_update_product_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("output_update_product.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_UPDATE_PRODUCT_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let output_update_product_ref01_ent = client.output_update_product(Value::Noval);
    let output_update_product_ref01_data = to_map(&getp(
        &getpath(&["new", "output_update_product"], &setup.data),
        "output_update_product_ref01",
    ));

    let output_update_product_ref01_data_result = output_update_product_ref01_ent
        .create(output_update_product_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let output_update_product_ref01_data = to_map(&output_update_product_ref01_data_result);
    assert!(
        matches!(output_update_product_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn output_update_product_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("output_update_product");
    entity_data_file.push("OutputUpdateProductTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("output_update_product01"), Value::str("output_update_product02"), Value::str("output_update_product03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_UPDATE_PRODUCT_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_UPDATE_PRODUCT_ENTID", idmap.clone()),
        ("BLUEFINTECSMERCHANTPORTAL_TEST_LIVE", Value::str("FALSE")),
        ("BLUEFINTECSMERCHANTPORTAL_TEST_EXPLAIN", Value::str("FALSE")),
    ]));

    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_UPDATE_PRODUCT_ENTID")) {
