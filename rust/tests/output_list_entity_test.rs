...
fn output_list_entity_basic() {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_LIST_ENTID JSON to run live");
        return;
    }
    ...
    let output_list_ref01_data = to_map(&output_list_ref01_data_result.data(None));
    assert!(
...
fn output_list_basic_setup(extra: Value) -> EntityTestSetup {
    ...
    let entid_env_raw = std::env::var("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_LIST_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_LIST_ENTID", idmap.clone()),
        ("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_LIVE", Value::str("FALSE")),
        ("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_EXPLAIN", Value::str("FALSE")),
    ]));

    let idmap_resolved = match to_map(&getp(&env, "BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_LIST_ENTID")) {
