// Generated direct-call tests for the output_message entity (mirrors the
...
struct OutputMessageDirectSetup {
...
fn output_message_direct_setup(mockres: Value) -> OutputMessageDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_MESSAGE_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_MESSAGE_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return OutputMessageDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn output_message_direct_load() {
    let setup = output_message_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-output_message", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["output_message01"] {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "output_message01"));
    } else {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("merchantportalws/batch/registerAdditionalTerminal/restart/{id}")),
