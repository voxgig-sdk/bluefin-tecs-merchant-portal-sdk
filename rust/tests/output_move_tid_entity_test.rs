...
#[test]
fn output_move_tid_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.output_move_tid(Value::Noval);
    assert_eq!(ent.get_name(), "output_move_tid");
}

#[test]
fn output_move_tid_entity_stream() {
    ...
    let plainent = plainsdk.output_move_tid(Value::Noval);
...
#[test]
fn output_move_tid_entity_basic() {
    let setup = output_move_tid_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("output_move_tid.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_MOVE_TID_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();
    // CREATE
    let output_move_tid_ref01_ent = client.output_move_tid(Value::Noval);
    let output_move_tid_ref01_data = to_map(&getp(
        &getpath(&["new", "output_move_tid"], &setup.data),
        "output_move_tid_ref01",
    ));

    let output_move_tid_ref01_data_result = output_move_tid_ref01_ent
        .create(output_move_tid_ref01_data.clone(), Value::Noval)
        .expect("create failed");
    let output_move_tid_ref01_data = to_map(&output_move_tid_ref01_data_result);
    assert!(
        matches!(output_move_tid_ref01_data, Value::Map(_)),
        "expected create result to be a map"
    );

}

fn output_move_tid_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("output_move_tid");
    entity_data_file.push("OutputMoveTidTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("output_move_tid01"), Value::str("output_move_tid02"), Value::str("output_move_tid03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_MOVE_TID_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_MOVE_TID_ENTID", idmap.clone()),
        ("BLUEFINTECSMERCHANTPORTAL_TEST_LIVE", Value::str("FALSE")),
        ("BLUEFINTECSMERCHANTPORTAL_TEST_EXPLAIN", Value::str("FALSE")),
    ]));

    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_MOVE_TID_ENTID")) {
