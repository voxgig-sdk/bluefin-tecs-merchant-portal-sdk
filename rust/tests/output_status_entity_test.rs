...
#[test]
fn output_status_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.output_status(Value::Noval);
    assert_eq!(ent.get_name(), "output_status");
}

#[test]
fn output_status_entity_stream() {
    ...
    let plainent = plainsdk.output_status(Value::Noval);
...
#[test]
fn output_status_entity_basic() {
    let setup = output_status_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["load"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("output_status.{}", op), mode);
        if skip {
    ...
    if setup.synthetic_only {
        eprintln!("skip: live entity test uses synthetic IDs from fixture — set BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_STATUS_ENTID JSON to run live");
        return;
    }
    let client = setup.client.clone();

    // Bootstrap entity data from existing test data (no create step in flow).
    let output_status_ref01_data_raw = vs::items(&to_map(&getpath(&["existing", "output_status"], &setup.data)));
    let output_status_ref01_data = to_map(&vs::get_elem(
        &vs::get_elem(&output_status_ref01_data_raw, &Value::Num(0.0), Value::Noval),
        &Value::Num(1.0),
        Value::Noval,
    ));
    // LOAD
    ...
    let output_status_ref01_data_dt0_loaded = output_status_ref01_ent
        ...
        .expect("load failed");
    assert!(
        !output_status_ref01_data_dt0_loaded.is_noval(),
        "expected load result to be non-nil"
    );

}

fn output_status_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("output_status");
    entity_data_file.push("OutputStatusTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("output_status01"), Value::str("output_status02"), Value::str("output_status03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
    ...
    let entid_env_raw = std::env::var("BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_STATUS_ENTID").unwrap_or_default();
    let idmap_overridden =
        !entid_env_raw.trim().is_empty() && entid_env_raw.trim().starts_with('{');

    let env = env_override(jo(vec![
        ("BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_STATUS_ENTID", idmap.clone()),
        ("BLUEFINTECSMERCHANTPORTAL_TEST_LIVE", Value::str("FALSE")),
        ("BLUEFINTECSMERCHANTPORTAL_TEST_EXPLAIN", Value::str("FALSE")),
    ]));

    let idmap_resolved = match to_map(&getp(&env, "BLUEFINTECSMERCHANTPORTAL_TEST_OUTPUT_STATUS_ENTID")) {
