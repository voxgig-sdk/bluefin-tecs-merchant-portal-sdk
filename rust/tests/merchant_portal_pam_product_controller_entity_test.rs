...
#[test]
fn merchant_portal_pam_product_controller_entity_instance() {
    let testsdk = test_sdk(Value::Noval, Value::Noval);
    let ent = testsdk.merchant_portal_pam_product_controller(Value::Noval);
    assert_eq!(ent.get_name(), "merchant_portal_pam_product_controller");
}

#[test]
fn merchant_portal_pam_product_controller_entity_stream() {
    ...
    let plainent = plainsdk.merchant_portal_pam_product_controller(Value::Noval);
...
#[test]
fn merchant_portal_pam_product_controller_entity_basic() {
    let setup = merchant_portal_pam_product_controller_basic_setup(Value::Noval);
    // Per-op sdk-test-control.json skip — the basic test exercises a flow
    // with multiple ops; skipping any op skips the whole flow.
    let mode = if setup.live { "live" } else { "unit" };
    for op in ["create"] {
        let (skip, reason) = is_control_skipped("entityOp", &format!("merchant_portal_pam_product_controller.{}", op), mode);
        if skip {
    ...
    // CREATE
    let merchant_portal_pam_product_controller_ref01_ent = client.merchant_portal_pam_product_controller(Value::Noval);
    let merchant_portal_pam_product_controller_ref01_data = to_map(&getp(
        &getpath(&["new", "merchant_portal_pam_product_controller"], &setup.data),
...
fn merchant_portal_pam_product_controller_basic_setup(extra: Value) -> EntityTestSetup {
    load_env_local();
    ...
    entity_data_file.push("merchant_portal_pam_product_controller");
    entity_data_file.push("MerchantPortalPamProductControllerTestData.json");
    ...
    let idmap = vs::transform(
        &ja(vec![Value::str("merchant_portal_pam_product_controller01"), Value::str("merchant_portal_pam_product_controller02"), Value::str("merchant_portal_pam_product_controller03")]),
        &jo(vec![(
            "`$PACK`",
            ja(vec![
                Value::str(""),
