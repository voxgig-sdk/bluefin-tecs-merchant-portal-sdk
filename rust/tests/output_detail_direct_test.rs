// Generated direct-call tests for the output_detail entity (mirrors the
...
struct OutputDetailDirectSetup {
...
fn output_detail_direct_setup(mockres: Value) -> OutputDetailDirectSetup {
    load_env_local();

    let calls: Rc<RefCell<Vec<Value>>> = Rc::new(RefCell::new(Vec::new()));

    let env = env_override(jo(vec![
        ("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_DETAIL_ENTID", Value::empty_map()),
    ...
    if live {
        ...
        let idmap = match to_map(&getp(&env, "BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_DETAIL_ENTID")) {
            Value::Map(m) => Value::Map(m),
            _ => Value::empty_map(),
        };
        return OutputDetailDirectSetup {
            client,
            calls,
            live: true,
            idmap,
...
#[test]
fn output_detail_direct_load() {
    let setup = output_detail_direct_setup(jo(vec![("id", Value::str("direct01"))]));
    let mode = if setup.live { "live" } else { "unit" };
    let (skip, reason) = is_control_skipped("direct", "direct-load-output_detail", mode);
    if skip {
        eprintln!(
            "skip: {}",
            if reason.is_empty() {
    ...
    if setup.live {
        for live_key in ["output_detail01"] {
    ...
    if setup.live {
        setp(&params, "id", getp(&setup.idmap, "output_detail01"));
    } else {
    ...
    let result = client
        .direct(jo(vec![
            ("path", Value::str("merchantportalws/batch/registerAdditionalTerminal/details/{id}")),
