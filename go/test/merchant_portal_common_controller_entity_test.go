...
func TestMerchantPortalCommonControllerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		merchantPortalCommonControllerRef01DataRaw := vs.Items(core.ToMapAny(vs.GetPath(setup.data, "existing.merchant_portal_common_controller")))
...
func merchant_portal_common_controllerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_COMMON_CONTROLLER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_COMMON_CONTROLLER_ENTID"])
