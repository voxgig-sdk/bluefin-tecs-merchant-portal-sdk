...
func TestMerchantPortalPamMandatorControllerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		merchantPortalPamMandatorControllerRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "merchant_portal_pam_mandator_controller"}), "merchant_portal_pam_mandator_controller_ref01"))
		...
		merchantPortalPamMandatorControllerRef01Data = core.ToMapAny(entityData(merchantPortalPamMandatorControllerRef01DataResult))
...
func merchant_portal_pam_mandator_controllerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_MANDATOR_CONTROLLER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_MANDATOR_CONTROLLER_ENTID"])
