...
func TestOutputStartEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		outputStartRef01Data := core.ToMapAny(vs.GetProp(
			vs.GetPath(setup.data, []any{"new", "output_start"}), "output_start_ref01"))

		outputStartRef01DataResult, err := outputStartRef01Ent.Create(outputStartRef01Data, nil)
		if err != nil {
			t.Fatalf("create failed: %v", err)
		}
		outputStartRef01Data = core.ToMapAny(entityData(outputStartRef01DataResult))
...
func output_startBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_START_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_START_ENTID"])
