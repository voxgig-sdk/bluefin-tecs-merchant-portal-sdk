...
func envOverride(m map[string]any) map[string]any {
	if os.Getenv("BLUEFINTECSMERCHANTPORTAL_TEST_LIVE") == "TRUE" ||
		os.Getenv("BLUEFINTECSMERCHANTPORTAL_TEST_OVERRIDE") == "TRUE" {
		for key := range m {
	...
	if explain := os.Getenv("BLUEFINTECSMERCHANTPORTAL_TEST_EXPLAIN"); explain != "" {
