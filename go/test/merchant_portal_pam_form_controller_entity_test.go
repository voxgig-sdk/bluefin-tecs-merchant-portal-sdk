...
func TestMerchantPortalPamFormControllerEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		merchantPortalPamFormControllerRef01Data = core.ToMapAny(entityData(merchantPortalPamFormControllerRef01DataResult))
...
func merchant_portal_pam_form_controllerBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_FORM_CONTROLLER_ENTID")
	...
	idmapResolved := core.ToMapAny(env["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_FORM_CONTROLLER_ENTID"])
