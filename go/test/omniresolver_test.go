...
import (
	...
	omni "github.com/voxgig-sdk/bluefin-tecs-merchant-portal-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.BluefinTecsMerchantPortalError); is {
				out := map[string]any{
					"name":    "BluefinTecsMerchantPortalError",
...
func omniCtx(arg any, client *sdk.BluefinTecsMerchantPortalSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.BluefinTecsMerchantPortalSDK); is {
			client = live
