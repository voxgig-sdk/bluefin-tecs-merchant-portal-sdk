...
//    only - a typed BluefinTecsMerchantPortalContext there would make every ctx
...
final class BluefinTecsMerchantPortalOmni
{
    ...
    public static function ctx_from_map($ctxmap, $client, $utility)
    {
        ...
        $ctx = new BluefinTecsMerchantPortalContext($clean, null);
        ...
        if (isset($clean['spec']) && is_array($clean['spec'])) {
            $ctx->spec = new BluefinTecsMerchantPortalSpec($clean['spec']);
        }

        if (isset($clean['result']) && is_array($clean['result'])) {
            $ctx->result = new BluefinTecsMerchantPortalResult($clean['result']);
            if (isset($clean['result']['err']) && is_array($clean['result']['err'])) {
                $msg = $clean['result']['err']['message'] ?? '';
                $ctx->result->err = new BluefinTecsMerchantPortalError('', is_string($msg) ? $msg : '');
            }
        }

        if (isset($clean['response']) && is_array($clean['response'])) {
            $ctx->response = new BluefinTecsMerchantPortalResponse($clean['response']);
    ...
    public static function err_from_map($m): ?BluefinTecsMerchantPortalError
    {
        if (!is_array($m)) {
            return null;
        }
        ...
        $code = $m['code'] ?? '';
        return new BluefinTecsMerchantPortalError(is_string($code) ? $code : '', $msg);
    }
