...
class OutputMoveTidEntityTest extends TestCase
{
    public function test_create_instance(): void
    {
        $testsdk = BluefinTecsMerchantPortalSDK::test(null, null);
        $ent = $testsdk->OutputMoveTid(null);
        $this->assertNotNull($ent);
    }

    public function test_basic_flow(): void
    {
        $setup = output_move_tid_basic_setup(null);
        // Per-op sdk-test-control.json skip.
        $_live = !empty($setup["live"]);
        foreach (["create"] as $_op) {
            [$_shouldSkip, $_reason] = Runner::is_control_skipped("entityOp", "output_move_tid." . $_op, $_live ? "live" : "unit");
            if ($_shouldSkip) {
        ...
        // CREATE
        $output_move_tid_ref01_ent = $client->OutputMoveTid(null);
...
function output_move_tid_basic_setup($extra)
{
    ...
    $entity_data_file = __DIR__ . '/../../.sdk/test/entity/output_move_tid/OutputMoveTidTestData.json';
    ...
    // Generate idmap.
    $idmap = [];
    foreach (["output_move_tid01", "output_move_tid02", "output_move_tid03"] as $k) {
