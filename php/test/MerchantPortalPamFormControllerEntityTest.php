...
class MerchantPortalPamFormControllerEntityTest extends TestCase
{
    ...
    public function test_basic_flow(): void
    {
        ...
        if (!empty($setup["synthetic_only"])) {
            $this->markTestSkipped("live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_FORM_CONTROLLER_ENTID JSON to run live");
            return;
        }
        $client = $setup["client"];
        ...
        $merchant_portal_pam_form_controller_ref01_data = Helpers::to_map(is_object($merchant_portal_pam_form_controller_ref01_data_result) && method_exists($merchant_portal_pam_form_controller_ref01_data_result, 'data_get') ? $merchant_portal_pam_form_controller_ref01_data_result->data_get() : $merchant_portal_pam_form_controller_ref01_data_result);
...
function merchant_portal_pam_form_controller_basic_setup($extra)
{
    ...
    $entid_env_raw = getenv("BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_FORM_CONTROLLER_ENTID");
