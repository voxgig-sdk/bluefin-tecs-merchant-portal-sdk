...
void tests() {
  describe('MerchantPortalApiControllerEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantPortalSDK.test();
      final ent = testsdk.MerchantPortalApiController();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.MerchantPortalApiController();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'merchant_portal_api_controller.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final merchant_portal_api_controller_ref01_ent = client.MerchantPortalApiController();
      dynamic merchant_portal_api_controller_ref01_data = setup['data']['new']['merchant_portal_api_controller']['merchant_portal_api_controller_ref01'];

      merchant_portal_api_controller_ref01_data = await merchant_portal_api_controller_ref01_ent.create(merchant_portal_api_controller_ref01_data);
      ok(null != merchant_portal_api_controller_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['merchant_portal_api_controller01', 'merchant_portal_api_controller02', 'merchant_portal_api_controller03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_API_CONTROLLER_ENTID'];
