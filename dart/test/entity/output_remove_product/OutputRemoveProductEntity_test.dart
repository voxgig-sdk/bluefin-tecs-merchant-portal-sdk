...
void tests() {
  describe('OutputRemoveProductEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantPortalSDK.test();
      final ent = testsdk.OutputRemoveProduct();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputRemoveProduct();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_remove_product.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final output_remove_product_ref01_ent = client.OutputRemoveProduct();
      dynamic output_remove_product_ref01_data = setup['data']['new']['output_remove_product']['output_remove_product_ref01'];

      output_remove_product_ref01_data = await output_remove_product_ref01_ent.create(output_remove_product_ref01_data);
      ok(null != output_remove_product_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_remove_product01', 'output_remove_product02', 'output_remove_product03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_REMOVE_PRODUCT_ENTID'];
