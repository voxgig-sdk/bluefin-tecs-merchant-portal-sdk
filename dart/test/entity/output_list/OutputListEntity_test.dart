...
void tests() {
  describe('OutputListEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputList();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_list.' + op, live)) {
          return;
        }
      }
      ...
      // CREATE
      final output_list_ref01_ent = client.OutputList();
      dynamic output_list_ref01_data = setup['data']['new']['output_list']['output_list_ref01'];

      output_list_ref01_data = await output_list_ref01_ent.create(output_list_ref01_data);
      ok(null != output_list_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_list01', 'output_list02', 'output_list03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_LIST_ENTID'];
