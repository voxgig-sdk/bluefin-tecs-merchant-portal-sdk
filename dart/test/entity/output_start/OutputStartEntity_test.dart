...
void tests() {
  describe('OutputStartEntity', () {
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.OutputStart();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'output_start.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_START_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final output_start_ref01_ent = client.OutputStart();
      dynamic output_start_ref01_data = setup['data']['new']['output_start']['output_start_ref01'];

      output_start_ref01_data = await output_start_ref01_ent.create(output_start_ref01_data);
      ok(null != output_start_ref01_data['id']);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['output_start01', 'output_start02', 'output_start03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_START_ENTID'];
