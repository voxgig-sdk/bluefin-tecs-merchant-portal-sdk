...
void tests() {
  describe('MerchantPortalPamMandatorControllerEntity', () {
    test('instance', (t) async {
      final testsdk = BluefinTecsMerchantPortalSDK.test();
      final ent = testsdk.MerchantPortalPamMandatorController();
      ok(null != ent);
    });
    ...
    test('stream', (t) async {
      ...
      final plainent = plainsdk.MerchantPortalPamMandatorController();
      final seen2 = [];
    ...
    test('basic', (t) async {
      ...
      for (final op in ['create']) {
        if (maybeSkipControl(t, 'entityOp', 'merchant_portal_pam_mandator_controller.' + op, live)) {
          return;
        }
      }
      ...
      if (true == setup['syntheticOnly']) {
        t.skip('live entity test uses synthetic IDs from fixture — set BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_MANDATOR_CONTROLLER_ENTID JSON to run live');
        return;
      }
      ...
      // CREATE
      final merchant_portal_pam_mandator_controller_ref01_ent = client.MerchantPortalPamMandatorController();
      dynamic merchant_portal_pam_mandator_controller_ref01_data = setup['data']['new']['merchant_portal_pam_mandator_controller']['merchant_portal_pam_mandator_controller_ref01'];

      merchant_portal_pam_mandator_controller_ref01_data = await merchant_portal_pam_mandator_controller_ref01_ent.create(merchant_portal_pam_mandator_controller_ref01_data);
      ok(null != merchant_portal_pam_mandator_controller_ref01_data);
...
Map<String, dynamic> basicSetup([dynamic extra]) {
  ...
  dynamic idmap = transform(
      <dynamic>['merchant_portal_pam_mandator_controller01', 'merchant_portal_pam_mandator_controller02', 'merchant_portal_pam_mandator_controller03'],
      <String, dynamic>{
        '`\$PACK`': <dynamic>[
          '',
          <String, dynamic>{
  ...
  final idmapEnvVal =
      Platform.environment['BLUEFIN_TECS_MERCHANT_PORTAL_TEST_MERCHANT_PORTAL_PAM_MANDATOR_CONTROLLER_ENTID'];
