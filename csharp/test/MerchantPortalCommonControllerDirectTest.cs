// merchant_portal_common_controller direct API tests (generated from the API model).
...
public class MerchantPortalCommonControllerDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = MerchantPortalCommonControllerDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-merchant_portal_common_controller", _mode);
        if (_shouldSkip)
        {
    ...
    private class MerchantPortalCommonControllerDirectSetupResult
    {
    ...
    private static MerchantPortalCommonControllerDirectSetupResult MerchantPortalCommonControllerDirectSetup(object? mockres)
    {
        ...
        if (live)
        {
            ...
            return new MerchantPortalCommonControllerDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
