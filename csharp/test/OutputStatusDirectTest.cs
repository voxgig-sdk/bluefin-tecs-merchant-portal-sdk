// output_status direct API tests (generated from the API model).
...
public class OutputStatusDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = OutputStatusDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-output_status", _mode);
        if (_shouldSkip)
        {
    ...
    private class OutputStatusDirectSetupResult
    {
    ...
    private static OutputStatusDirectSetupResult OutputStatusDirectSetup(object? mockres)
    {
        ...
        if (live)
        {
            ...
            return new OutputStatusDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
