...
public class PipelineTest
{
    ...
    private static (BluefinTecsMerchantPortalSDK, Utility) PlClient(Dictionary<string, object?>? sdkopts)
    {
        var client = BluefinTecsMerchantPortalSDK.TestSDK(null, sdkopts);
        return (client, client.GetUtility());
    }

    private static Context PlCtx(BluefinTecsMerchantPortalSDK client, Utility utility,
    ...
    private static string ErrCode(Action act)
    {
        ...
        catch (BluefinTecsMerchantPortalError err)
        {
            return err.Code;
        }
        catch (Exception)
    ...
    private class PlEntity : IEntity
    {
        ...
        // BluefinTecsMerchantPortalEntityBase: `remove` marks the instance, which keeps the
    ...
    private static Utility UtilWith(BluefinTecsMerchantPortalSDK client, FetcherFunc fetcher)
    {
    ...
    public void MakeRequestTransportErrorCarriedOnResponse()
    {
        ...
        Assert.True(resp.Err is BluefinTecsMerchantPortalError pe && pe.Code == "boom",
