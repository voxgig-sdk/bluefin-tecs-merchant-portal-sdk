// output_detail direct API tests (generated from the API model).
...
public class OutputDetailDirectTest
{
    [Fact]
    public void DirectLoad()
    {
        var setup = OutputDetailDirectSetup(
            new Dictionary<string, object?> { ["id"] = "direct01" });
        var _mode = setup.Live ? "live" : "unit";
        var (_shouldSkip, _) = TestRunner.IsControlSkipped(
            "direct", "direct-load-output_detail", _mode);
        if (_shouldSkip)
        {
    ...
    private class OutputDetailDirectSetupResult
    {
    ...
    private static OutputDetailDirectSetupResult OutputDetailDirectSetup(object? mockres)
    {
        ...
        {
            ["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_DETAIL_ENTID"] = new Dictionary<string, object?>(),
        ...
        if (live)
        {
            ...
            var entidRaw = env["BLUEFIN_TECS_MERCHANT_PORTAL_TEST_OUTPUT_DETAIL_ENTID"];
            ...
            return new OutputDetailDirectSetupResult
            {
                Client = liveClient,
                Calls = calls,
                Live = true,
