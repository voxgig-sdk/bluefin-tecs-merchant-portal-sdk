...
enum OmniResolver {
  ...
  private final class ProviderBox {
    var clients: [ObjectIdentifier: BluefinTecsMerchantPortalSDK] = [:]
  ...
  private static func register(_ provider: Omni.Provider, _ client: BluefinTecsMerchantPortalSDK,
    _ isdef: Bool)
  {
    providers.lock.lock()
  ...
  private static func defclient(_ provider: Omni.Provider) -> BluefinTecsMerchantPortalSDK? {
    providers.lock.lock()
    defer { providers.lock.unlock() }
  ...
  private static func sdkProvider(_ client: BluefinTecsMerchantPortalSDK, _ isdef: Bool = false)
    -> Omni.Provider
  {
    ...
    provider.client = { options in
      ...
      return sdkProvider(BluefinTecsMerchantPortalSDK.testSDK(nil, opts), true)
    }

    // Client options may reference the runner store.
    provider.inject = { options, store in
      let o = tovalue(options)
      _ = BluefinTecsMerchantPortalSdk.inject(o, tovalue(store))
      return tojson(o)
    }
    ...
    provider.errify = { err in
      if let sdkerr = err as? BluefinTecsMerchantPortalError {
        var entries: [(String, Omni.Json)] = [
          ("name", .str("BluefinTecsMerchantPortalError")),
  ...
  final class Run {
    ...
    fileprivate init(_ pack: Omni.RunPack, _ client: BluefinTecsMerchantPortalSDK) {
      self.pack = pack
  ...
  /// itself).
  static func makeRunner(_ testfile: String, _ client: BluefinTecsMerchantPortalSDK) throws
    -> NamedRunner
  {
  ...
  static func makeRunner(_ spec: Value, _ client: BluefinTecsMerchantPortalSDK) throws
    -> NamedRunner
  {
    return try namedRunner(try Omni.makeRunner(tojson(spec), sdkProvider(client)), client)
  }

  private static func namedRunner(_ runner: Omni.Runner, _ client: BluefinTecsMerchantPortalSDK)
    -> NamedRunner
  {
  ...
  static func omniCtx(_ arg: Value, _ client: BluefinTecsMerchantPortalSDK, _ utility: Utility)
    -> Context
  {
