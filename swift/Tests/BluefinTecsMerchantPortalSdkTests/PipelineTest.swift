...
final class PipelineTest: XCTestCase {
  ...
  private func plClient(_ sdkopts: VMap?) -> (BluefinTecsMerchantPortalSDK, Utility) {
    let client = BluefinTecsMerchantPortalSDK.testSDK(nil, sdkopts)
    return (client, client.getUtility())
  }

  private func plCtx(_ client: BluefinTecsMerchantPortalSDK, _ utility: Utility, _ ctrl: VMap?) -> Context {
  ...
  private func errCode(_ act: () throws -> Void) -> String {
    do { try act(); return "" } catch let e as BluefinTecsMerchantPortalError { return e.code } catch { return "" }
  }
  ...
  private func utilWith(_ client: BluefinTecsMerchantPortalSDK, _ fetcher: @escaping FetcherFunc) -> Utility {
  ...
  func testMakeRequestTransportErrorCarriedOnResponse() {
    ...
    XCTAssertEqual((resp.err as? BluefinTecsMerchantPortalError)?.code, "boom")
  }
