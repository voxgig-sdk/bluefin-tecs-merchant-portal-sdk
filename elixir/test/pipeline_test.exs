...
defmodule BluefinTecsMerchantPortal.PipelineFakeEntity do
...
defmodule BluefinTecsMerchantPortal.PipelineTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinTecsMerchantPortal.Helpers, as: H
  alias BluefinTecsMerchantPortal.{Utility, Context, Spec, Result, Response}

  defp client, do: BluefinTecsMerchantPortal.test()

  defp make_ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname, "ctrl", S.jm([])]), BluefinTecsMerchantPortal.get_root_ctx(client))
  end
  ...
  defp util_override(c, key, fun) do
    S.setprop(BluefinTecsMerchantPortal.get_utility(c), key, fun)
    c
  end
  ...
  defp resolve_order(feature) do
    ctx = Context.new(S.jm(["opname", "load", "ctrl", S.jm([])]), BluefinTecsMerchantPortal.get_root_ctx(client()))
  ...
  defp list_ctx(c, collector) do
    entity =
      S.jm(["_module", BluefinTecsMerchantPortal.PipelineFakeEntity, "_name", "widget", "_collector", collector])

    Context.new(S.jm(["opname", "list", "entity", entity, "ctrl", S.jm([])]), BluefinTecsMerchantPortal.get_root_ctx(c))
  end
  ...
  test "make_request a transport error tuple lands on the response" do
    boom = BluefinTecsMerchantPortal.Error.new("boom", "boom")
  ...
  test "make_request a fetchdef error surfaces as a response error" do
    c = util_override(client(), "make_fetch_def", fn _ctx -> {nil, BluefinTecsMerchantPortal.Error.new("fetchdef_boom", "boom")} end)
    ctx = make_ctx(c)
    S.setprop(ctx, "spec", request_spec())
    {response, err} = Utility.make_request(ctx)
    assert err == nil
  ...
  test "done raises the error when not ok" do
    ...
    assert_raise BluefinTecsMerchantPortal.Error, fn -> Utility.done(ctx) end
  end
  ...
  test "make_error preserves the error code" do
    ...
    err =
      assert_raise BluefinTecsMerchantPortal.Error, fn ->
  ...
  test "feature_add appends in call order" do
    ...
    a = BluefinTecsMerchantPortal.Feature.new()
    b = BluefinTecsMerchantPortal.Feature.new()
  ...
  defp named_feature(name) do
    f = BluefinTecsMerchantPortal.Feature.new()
  ...
  defp auth_ctx(c, options, headers) do
    ...
    ctx =
      Context.new(
        S.jm(["opname", "load", "client", fakeclient, "utility", BluefinTecsMerchantPortal.get_utility(c), "ctrl", S.jm([])]),
        nil
      )
