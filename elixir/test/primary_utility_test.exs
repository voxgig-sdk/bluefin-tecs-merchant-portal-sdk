...
defmodule BluefinTecsMerchantPortal.PrimaryUtilityTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinTecsMerchantPortal.Omni, as: O
  alias BluefinTecsMerchantPortal.Utility, as: U
  ...
  defp livectx(node, opts) do
    ...
    util = BluefinTecsMerchantPortal.get_utility(sdk)
    rootctx = BluefinTecsMerchantPortal.get_root_ctx(sdk)

    ctxmap = S.clone(node)
    S.delprop(ctxmap, "client")
    S.delprop(ctxmap, "utility")

    ctx = BluefinTecsMerchantPortal.Context.new(ctxmap, nil)
    ...
    if S.ismap(specmap), do: S.setprop(ctx, "spec", BluefinTecsMerchantPortal.Spec.new(specmap))

    resmap = S.getprop(ctxmap, "result")

    if S.ismap(resmap) do
      result = BluefinTecsMerchantPortal.Result.new(resmap)
      ...
      if S.ismap(errmap) do
        ...
        if is_binary(msg) and msg != "",
          do: S.setprop(result, "err", BluefinTecsMerchantPortal.Error.new("", msg, nil))
      end
    ...
    if S.ismap(respmap) do
      response = BluefinTecsMerchantPortal.Response.new(respmap)
  ...
  test "the primary corpus runs through the vendored omni engine", %{run: run, sdk: sdk} do
    ...
    runsection.("makeContext", fn args ->
      ...
      out = BluefinTecsMerchantPortal.Context.new(ctxmap, nil)
    ...
    specsdk = BluefinTecsMerchantPortal.test(nil, setup_opts(spec, "makeSpec"))
    ...
    runsection.("operator", fn args ->
      op = BluefinTecsMerchantPortal.Operation.new(argnode(args, 0))
    ...
    authsdk = BluefinTecsMerchantPortal.test(nil, setup_opts(spec, "prepareAuth"))
