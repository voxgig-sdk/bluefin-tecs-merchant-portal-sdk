...
# BluefinTecsMerchantPortal.FeatureHarness — the same hook order and short-circuit rules as
...
defmodule BluefinTecsMerchantPortal.FeatureTest do
  use ExUnit.Case

  alias Voxgig.Struct, as: S
  alias BluefinTecsMerchantPortal.Helpers, as: H
  alias BluefinTecsMerchantPortal.FeatureHarness, as: FH
    ...
    if has?("retry") do
      ...
      {server, calls} =
        FH.recording_server(fn n, _fd ->
          if n < 3,
            do: {nil, BluefinTecsMerchantPortal.Error.new("boom", "boom")},
  ...
  test "retry exhausted transport error surfaces" do
    if has?("retry") do
      clock = FH.make_clock()
      {server, calls} = FH.recording_server(fn _n, _fd -> {nil, BluefinTecsMerchantPortal.Error.new("boom", "boom")} end)
