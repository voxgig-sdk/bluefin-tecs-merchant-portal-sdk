# BluefinTecsMerchantPortal SDK utility
#
...
defmodule BluefinTecsMerchantPortal.Utility do
  alias Voxgig.Struct, as: S
  alias BluefinTecsMerchantPortal.Helpers, as: H
  alias BluefinTecsMerchantPortal.{Context, Spec, Result, Response, Operation}

  @default_user_agent "Mozilla/5.0 (compatible; BluefinTecsMerchantPortalSDK/1.0)"
  ...
  def make_options_impl(ctx) do
    ...
    # (BluefinTecsMerchantPortal.Config.shared_config) and merge uses its nested nodes as
      ...
      resolved =
        Regex.replace(~r/\{([A-Za-z0-9_]+)\}/, base, fn _match, name ->
          ...
          cond do
            ...
            true ->
              raise BluefinTecsMerchantPortal.Error,
  ...
  def make_error_impl(ctx, err) do
    ...
    errmsg =
      cond do
        match?(%BluefinTecsMerchantPortal.Error{}, err) -> err.msg
    ...
    msg = "BluefinTecsMerchantPortalSDK: " <> opname <> ": " <> (errmsg || "")
    msg = clean(ctx, msg)
    ...
    sdk_err = %BluefinTecsMerchantPortal.Error{
      code: "",
      msg: msg,
      sdk: "BluefinTecsMerchantPortal",
      ctx: ctx,
    ...
    sdk_err = if match?(%BluefinTecsMerchantPortal.Error{}, err), do: %{sdk_err | code: err.code}, else: sdk_err
  ...
  defp err_msg(e) do
    cond do
      match?(%BluefinTecsMerchantPortal.Error{}, e) -> e.msg
  ...
  defp safe_json(str) do
    try do
      {:ok, BluefinTecsMerchantPortal.Json.parse(str)}
    rescue
      _ -> :error
    end
  end
