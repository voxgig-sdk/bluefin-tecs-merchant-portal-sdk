// Generated direct-call tests for the output_message entity (mirrors the java
...
object OutputMessageDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-output_message") {
      ...
      rep.eq("direct-load-output_message.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-output_message.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-output_message.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-output_message.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-output_message.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-output_message.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
