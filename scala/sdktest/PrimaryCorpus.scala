...
import voxgig.bluefintecsmerchantportalsdk.core.*
...
object PrimaryCorpus {
  ...
  def toValue(o: Object): Value = o match {
    ...
    case x if x eq voxgig.bluefintecsmerchantportalsdk.utility.struct.Struct.UNDEF => VNull
    case m: JMap[_, _] =>
  ...
  private def corpusCtx(client: BluefinTecsMerchantPortalSDK, ctxmap: Value): Context = {
  ...
  private def runset(name: String, client: BluefinTecsMerchantPortalSDK)(f: (Context, Seq[Value]) => Value): Unit = {
  ...
  private def clientFor(name: String, shared: BluefinTecsMerchantPortalSDK): BluefinTecsMerchantPortalSDK =
    sectionSetup(sectionRun(name)) match {
      case m @ VMap(_) => BluefinTecsMerchantPortalSDK.testSDK(null, jmap(m))
      case _           => shared
    }
  ...
  def run(testfile: String): Int = {
    ...
    val sdk = BluefinTecsMerchantPortalSDK.testSDK()
