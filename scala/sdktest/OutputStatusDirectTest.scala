// Generated direct-call tests for the output_status entity (mirrors the java
...
object OutputStatusDirectTest {
  ...
  def run(rep: SdkTestReport): Unit = {
    rep.scope("direct-load-output_status") {
      ...
      rep.eq("direct-load-output_status.ok", java.lang.Boolean.TRUE, result.get("ok"))
      rep.eqI("direct-load-output_status.status", 200, Helpers.toInt(result.get("status")))
      rep.check("direct-load-output_status.data", result.get("data") != null, "expected data to be non-null")
      val dataMap = Helpers.toMapAny(result.get("data"))
      if (dataMap != null) rep.eq("direct-load-output_status.dataId", "direct01", dataMap.get("id"))
      rep.eqI("direct-load-output_status.calls", 1, setup.calls.size())
      val url = setup.calls.get(0).get("url") match { case s: String => s; case _ => "" }
      rep.check("direct-load-output_status.url1", url.contains("direct01"), "expected url to contain direct01, got " + url)
    }
  }
}
