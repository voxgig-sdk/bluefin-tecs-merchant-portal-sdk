...
module BluefinTecsMerchantPortalUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = BluefinTecsMerchantPortalResponse.new({})
    ...
    if fetch_err
      ...
      response = BluefinTecsMerchantPortalResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = BluefinTecsMerchantPortalResponse.new(fetched)
    else
