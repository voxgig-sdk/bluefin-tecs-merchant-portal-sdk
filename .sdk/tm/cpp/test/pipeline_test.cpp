// BluefinTecsMerchantPortal SDK — direct unit tests for the operation-pipeline utilities
...
static std::shared_ptr<BluefinTecsMerchantPortalSDK> plClient(const Value& sdkopts) {
  return BluefinTecsMerchantPortalSDK::testSDK(Value::undef(), sdkopts);
}

static CtxPtr plCtx(std::shared_ptr<BluefinTecsMerchantPortalSDK> client, UtilityPtr utility, const Value& ctrl) {
  CtxSpec cs;
  cs.setOpname("load");
...
static std::string names(std::shared_ptr<BluefinTecsMerchantPortalSDK> client) {
  std::string out;
