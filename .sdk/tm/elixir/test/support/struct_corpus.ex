# Vendored struct corpus runner for the BluefinTecsMerchantPortal SDK.
#
...
defmodule BluefinTecsMerchantPortal.StructCorpus do
  ...
  defp pclient_for(primary, name) do
    ...
    if S.ismap(setup), do: BluefinTecsMerchantPortal.test(nil, S.clone(setup)), else: BluefinTecsMerchantPortal.test()
  end
  ...
  def run_primary(testfile \\ nil) do
    ...
    client = BluefinTecsMerchantPortal.test()
    Process.put(:pclient, client)
    Process.put(:pbaseclient, client)
    rootctx = BluefinTecsMerchantPortal.get_root_ctx(client)
    ...
    run_primary_set("makeContext", psec(primary, "makeContext"), fn e ->
      ...
      BluefinTecsMerchantPortal.Context.new(ctxmap, nil)
    end)
    ...
    run_primary_set("operator", psec(primary, "operator"), fn e ->
      inv = eget(e, "in")
      op = BluefinTecsMerchantPortal.Operation.new(if(S.ismap(inv), do: inv, else: S.jm([])))
      S.jm([
