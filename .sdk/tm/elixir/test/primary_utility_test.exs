...
defmodule BluefinTecsMerchantPortal.PrimaryUtilityTest do
  ...
  alias BluefinTecsMerchantPortal.{Context, Utility, Spec, Result}

  defp client, do: BluefinTecsMerchantPortal.test()

  defp ctx(client, opname \\ "load") do
    Context.new(S.jm(["opname", opname]), BluefinTecsMerchantPortal.get_root_ctx(client))
  end
  ...
  test "prepare_auth sets an authorization header when an apikey is present" do
    c = BluefinTecsMerchantPortal.new(H.deep(%{"apikey" => "secret"}))
    ctx = ctx(c)
  ...
  test "prepare_auth omits the header when no apikey" do
    c = BluefinTecsMerchantPortal.new()
    ctx = ctx(c)
  ...
  test "make_options derives a clean key regex" do
    c = BluefinTecsMerchantPortal.test()
    opts = BluefinTecsMerchantPortal.options_map(c)
