...
my $runner = BluefinTecsMerchantPortalOmni::make_runner($corpus_path, $client);
my $R = $runner->('struct');
...
my $NULLMARK = BluefinTecsMerchantPortalOmni::NULLMARK();
...
{
  my $td = BluefinTecsMerchantPortalOmni::tostruct(
...
{
  my $basic = BluefinTecsMerchantPortalOmni::tostruct(
...
grun('inject.string', $inject_spec->{string},
  sub { $S->{inject}->($_[0]{val}, $_[0]{store},
    { modify => \&BluefinTecsMerchantPortalOmni::null_modifier }) });
