...
#    `contextify` answers a TIED plain hash (BluefinTecsMerchantPortalOmniViewTie) whose
...
package BluefinTecsMerchantPortalOmniEntityRef;
...
# --------------------------------------------------------------------------
package BluefinTecsMerchantPortalOmniViewTie;
...
sub _iserr { return Scalar::Util::blessed($_[0]) && $_[0]->isa('BluefinTecsMerchantPortalError') }
...
sub _wrap {
  ...
  if ('ok' eq $name && Scalar::Util::blessed($t) && $t->isa('BluefinTecsMerchantPortalResult')
    && defined $val && !ref $val) {
  ...
  if (ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsMerchantPortalOmniViewTie')) {
    my $inner = tied(%$val)->{target};
    return $inner if $seen->{ Scalar::Util::refaddr($inner) };
    return BluefinTecsMerchantPortalOmni::objview($inner,
  ...
  if (Scalar::Util::blessed($val)) {
    ...
    if (('HASH' eq (Scalar::Util::reftype($val) || ''))) {
      ...
      return BluefinTecsMerchantPortalOmni::objview($val,
        { %$seen, Scalar::Util::refaddr($t) => 1 });
    }
  ...
  if (ref($val) eq 'HASH' || ref($val) eq 'ARRAY') {
    return BluefinTecsMerchantPortalOmni::toomni($val);
  }
...
# --------------------------------------------------------------------------
package BluefinTecsMerchantPortalOmniProviderTie;
...
package BluefinTecsMerchantPortalOmniProvider;
...
# The resolver proper.
# --------------------------------------------------------------------------
package BluefinTecsMerchantPortalOmni;
...
sub objview {
  my ($target, $seen) = @_;
  tie my %view, 'BluefinTecsMerchantPortalOmniViewTie', $target, $seen;
  return \%view;
}
...
sub isview {
  ...
  return ref($val) eq 'HASH' && tied(%$val)
    && tied(%$val)->isa('BluefinTecsMerchantPortalOmniViewTie') ? 1 : 0;
}
...
sub enrich {
  ...
  if (Voxgig::Struct::ismap($ctxmap->{spec}) && !Scalar::Util::blessed($ctxmap->{spec})) {
    $ctx->{spec} = BluefinTecsMerchantPortalSpec->new($ctxmap->{spec});
  }

  my $res_map = $ctxmap->{result};
  if (Voxgig::Struct::ismap($res_map) && !Scalar::Util::blessed($res_map)) {
    $ctx->{result} = BluefinTecsMerchantPortalResult->new($res_map);
    if (Voxgig::Struct::ismap($res_map->{err})
      && defined $res_map->{err}{message} && !ref $res_map->{err}{message}) {
      $ctx->{result}{err} = BluefinTecsMerchantPortalError->new('', $res_map->{err}{message});
    }
  }

  my $resp_map = $ctxmap->{response};
  if (Voxgig::Struct::ismap($resp_map) && !Scalar::Util::blessed($resp_map)) {
    $ctx->{response} = BluefinTecsMerchantPortalResponse->new($resp_map);
    if (BluefinTecsMerchantPortalHelpers::rb_truthy($resp_map->{body})) {
...
sub sdkhooks {
  ...
  my $struct = $utility->{struct} || BluefinTecsMerchantPortalHelpers::struct_facade();
  ...
  my $subject = sub {
    ...
    $found = $utility->{ BluefinTecsMerchantPortalOmniViewTie::_snake($name) } if !defined $found;
  ...
  my $contextify = sub {
    ...
    if (Voxgig::Struct::ismap($ent) && !Scalar::Util::blessed($ent)
      ...
      $ctxmap->{entity} = BluefinTecsMerchantPortalOmniEntityRef->new($ent->{name});
    }
    my $ctx = BluefinTecsMerchantPortalContext->new($ctxmap, undef);
    enrich($ctxmap, $ctx);
    $ctx->{utility} = $utility;
...
sub sdkprovider {
  my ($sdk) = @_;
  tie my %provider, 'BluefinTecsMerchantPortalOmniProviderTie', $sdk, sdkhooks($sdk);
  return bless \%provider, 'BluefinTecsMerchantPortalOmniProvider';
}
