...
sub smoke_pack {
  my $runner = BluefinTecsMerchantPortalOmni::make_runner($SPEC, BluefinTecsMerchantPortalSDK->test(undef, undef));
  return $runner->('smoke');
}
...
# A wrong result fails, with an OmniError naming the mismatch.
{
  ...
  my $err = $@;
  ok(BluefinTecsMerchantPortalOmni::is_omni_error($err), 'the failure is an OmniError');
...
# must fail.
{
  ...
  my $err = $@;
  ok(BluefinTecsMerchantPortalOmni::is_omni_error($err), 'the missing-error failure is an OmniError');
