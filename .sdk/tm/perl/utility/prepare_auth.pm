# BluefinTecsMerchantPortal SDK utility: prepare_auth
...
$REGISTRY{prepare_auth} = sub {
  ...
  if (!defined BluefinTecsMerchantPortalHelpers::gp($options, 'auth')) {
  ...
  if (!defined $apikey || Voxgig::Struct::is_none($apikey)
    ...
    my $auth_prefix = BluefinTecsMerchantPortalHelpers::gpath($options, 'auth.prefix');
