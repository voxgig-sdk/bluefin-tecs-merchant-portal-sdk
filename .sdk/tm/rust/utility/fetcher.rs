...
fn default_http_fetch(fullurl: &str, fetchdef: &Value) -> Result<Value, BluefinTecsMerchantPortalError> {
    ...
    if !has_ua {
        req = req.set("User-Agent", "Mozilla/5.0 (compatible; BluefinTecsMerchantPortalSDK/1.0)");
    }
    ...
    let resp = match sent {
        ...
        Err(e) => {
            return Err(BluefinTecsMerchantPortalError::new("fetch_transport", &format!("{}", e)));
        }
    };
    ...
    let body_txt = resp
        .into_string()
        .map_err(|e| BluefinTecsMerchantPortalError::new("fetch_body", &format!("{}", e)))?;
