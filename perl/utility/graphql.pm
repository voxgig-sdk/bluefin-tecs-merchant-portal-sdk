# BluefinTecsMerchantPortal SDK utility: graphql
#
...
sub graphql_error_code {
  ...
  my $ext = BluefinTecsMerchantPortalHelpers::gp($gqlerr, 'extensions');

  my $raw = BluefinTecsMerchantPortalHelpers::gp($ext, 'code');
  $raw = BluefinTecsMerchantPortalHelpers::gp($ext, 'type') if !defined $raw || '' eq $raw;
...
$REGISTRY{graphql_body} = sub {
  ...
  my $gql = BluefinTecsMerchantPortalHelpers::gp($ctx->{point}, 'graphql');
  ...
  my $varlist = BluefinTecsMerchantPortalHelpers::gp($gql, 'vars');
  $varlist = [] unless ref($varlist) eq 'ARRAY';

  for my $spec (@$varlist) {
    next unless ref($spec) eq 'HASH';

    my $name = BluefinTecsMerchantPortalHelpers::gp($spec, 'name');
    my $from = BluefinTecsMerchantPortalHelpers::gp($spec, 'from');
    ...
    my $val = BluefinTecsMerchantPortalHelpers::gp($reqsrc, $from);
    $val = BluefinTecsMerchantPortalHelpers::gp($datasrc, $from) unless defined $val;
    $variables{$name} = $val if defined $val;
  }

  return {
    query     => BluefinTecsMerchantPortalHelpers::gp($gql, 'doc'),
    variables => \%variables,
  };
};
...
$REGISTRY{graphql_errors} = sub {
  ...
  my $kind = BluefinTecsMerchantPortalHelpers::gp($ctx->{point}, 'kind');
  return 0 unless defined $kind && 'graphql' eq $kind;

  my $errors = BluefinTecsMerchantPortalHelpers::gp($result->{body}, 'errors');
  return 0 unless ref($errors) eq 'ARRAY' && 0 < scalar(@$errors);

  my $first = $errors->[0];
  my $msg = BluefinTecsMerchantPortalHelpers::gp($first, 'message');
