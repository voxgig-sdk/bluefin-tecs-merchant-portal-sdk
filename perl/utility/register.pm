...
$BluefinTecsMerchantPortalUtility::REGISTRAR = sub {
  my ($u) = @_;
  for my $k (keys %BluefinTecsMerchantPortalUtilities::REGISTRY) {
    $u->{$k} = $BluefinTecsMerchantPortalUtilities::REGISTRY{$k};
  }
  # The vendored struct utility, reachable as $utility->{struct}{<fn>}.
  $u->{struct} = BluefinTecsMerchantPortalHelpers::struct_facade();
};
