# BluefinTecsMerchantPortal SDK utility: make_point
...
sub _parts_len {
  my ($point) = @_;
  my $parts = BluefinTecsMerchantPortalHelpers::gp($point, 'parts');
...
$REGISTRY{make_point} = sub {
  ...
  if ($ctx->{out}{point}) {
    ...
    return (undef, $preset)
      if Scalar::Util::blessed($preset) && $preset->isa('BluefinTecsMerchantPortalError');
    $ctx->{point} = $preset;
  ...
  my $allow_op = BluefinTecsMerchantPortalHelpers::gpath($options, 'allow.op');
  ...
  if (1 == @{ $op->{points} }) {
    ...
    my $matched = 0;
    for my $p (@{ $op->{points} }) {
      my $select_def = BluefinTecsMerchantPortalHelpers::to_map(BluefinTecsMerchantPortalHelpers::gp($p, 'select'));
      my $found = 1;

      if ($selector && $select_def) {
        my $exist = BluefinTecsMerchantPortalHelpers::gp($select_def, 'exist');
        if (Voxgig::Struct::islist($exist)) {
          for my $ek (@$exist) {
            my $rv = BluefinTecsMerchantPortalHelpers::gp($reqselector, "$ek");
            my $sv = BluefinTecsMerchantPortalHelpers::gp($selector, "$ek");
      ...
      if ($found) {
        my $req_action = BluefinTecsMerchantPortalHelpers::gp($reqselector, '$action');
        my $select_action = BluefinTecsMerchantPortalHelpers::gp($select_def, '$action');
        $found = 0 unless BluefinTecsMerchantPortalHelpers::eqv($req_action, $select_action);
      }
    ...
    unless ($matched) {
      ...
      my $unmatched_action = $reqselector
        ? BluefinTecsMerchantPortalHelpers::gp($reqselector, '$action') : undef;
    ...
    if ($reqselector) {
      ...
      if (defined $req_action && $point) {
        my $point_select = BluefinTecsMerchantPortalHelpers::to_map(BluefinTecsMerchantPortalHelpers::gp($point, 'select'));
        my $point_action = BluefinTecsMerchantPortalHelpers::gp($point_select, '$action');
        unless (BluefinTecsMerchantPortalHelpers::eqv($req_action, $point_action)) {
          return (undef, $ctx->make_error('point_action_invalid',
            "Operation \"$op->{name}\" action \""
