# BluefinTecsMerchantPortal SDK utility: make_spec
...
$REGISTRY{make_spec} = sub {
  ...
  my $base = BluefinTecsMerchantPortalHelpers::gp($options, 'base');
  $base = '' unless defined $base;
  my $prefix = BluefinTecsMerchantPortalHelpers::gp($options, 'prefix');
  $prefix = '' unless defined $prefix;
  my $suffix = BluefinTecsMerchantPortalHelpers::gp($options, 'suffix');
  $suffix = '' unless defined $suffix;

  my $parts = [];
  $parts = BluefinTecsMerchantPortalHelpers::gp($point, 'parts') if $point;
  ...
  my $allow_method = BluefinTecsMerchantPortalHelpers::gpath($options, 'allow.method');
  ...
  my $pkind = BluefinTecsMerchantPortalHelpers::gp($ctx->{point}, 'kind');
  if (defined $pkind && 'graphql' eq $pkind) {
    ...
    $ctx->{spec}{headers}{'content-type'} =
      $BluefinTecsMerchantPortalUtilities::GRAPHQL_CONTENT_TYPE;
  }
  else {
