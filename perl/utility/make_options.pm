# BluefinTecsMerchantPortal SDK utility: make_options
...
$REGISTRY{make_options} = sub {
  ...
  my $custom_utils = BluefinTecsMerchantPortalHelpers::gp($options, 'utility');
  ...
  my $extend_raw = BluefinTecsMerchantPortalHelpers::gp($options, 'extend');
  ...
  my @featureorder;
  my $feature_raw = BluefinTecsMerchantPortalHelpers::gp($opts, 'feature');
  if (Voxgig::Struct::islist($feature_raw)) {
    my %fmap;
    for my $entry (@$feature_raw) {
      next unless Voxgig::Struct::ismap($entry);
      my $name = BluefinTecsMerchantPortalHelpers::gp($entry, 'name');
  ...
  BluefinTecsMerchantPortalHelpers::coerce_bools($opts);
  ...
  my $sys_fetch = BluefinTecsMerchantPortalHelpers::gpath($opts, 'system.fetch');

  # CLONE the config side: `config` is a process-wide singleton
  # (BluefinTecsMerchantPortalConfig::shared_config) and merge uses its nested hashes as merge
  ...
  my $clean_keys = BluefinTecsMerchantPortalHelpers::gpath($opts, 'clean.keys');
  ...
  if (!@featureorder) {
    my $fmap = BluefinTecsMerchantPortalHelpers::to_map(
      BluefinTecsMerchantPortalHelpers::gp($opts, 'feature')) || {};
