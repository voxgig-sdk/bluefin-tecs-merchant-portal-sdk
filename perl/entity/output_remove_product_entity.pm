# BluefinTecsMerchantPortal SDK OutputRemoveProduct entity
...
package OutputRemoveProductEntity;
...
sub make {
  ...
  return OutputRemoveProductEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputRemoveProduct data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputRemoveProduct fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputRemoveProduct.
#
# reqdata: body data hashref (OutputRemoveProductCreateData shape). ctrl: optional
# per-call control. Returns the created OutputRemoveProduct data (hashref); dies
