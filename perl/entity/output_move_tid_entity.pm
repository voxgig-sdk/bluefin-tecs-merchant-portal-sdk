# BluefinTecsMerchantPortal SDK OutputMoveTid entity
...
sub make {
  ...
  return OutputMoveTidEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputMoveTid data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputMoveTid fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputMoveTid.
#
# reqdata: body data hashref (OutputMoveTidCreateData shape). ctrl: optional
# per-call control. Returns the created OutputMoveTid data (hashref); dies
