# BluefinTecsMerchantPortal SDK OutputAddProduct entity
...
package OutputAddProductEntity;
...
sub make {
  ...
  return OutputAddProductEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputAddProduct data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputAddProduct fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputAddProduct.
#
# reqdata: body data hashref (OutputAddProductCreateData shape). ctrl: optional
# per-call control. Returns the created OutputAddProduct data (hashref); dies
