# BluefinTecsMerchantPortal SDK OutputStatus entity
...
sub make {
  ...
  return OutputStatusEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputStatus data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputStatus fields).
sub match_get {
  my ($self) = @_;
...
# Load a single OutputStatus.
#
# reqmatch: match criteria hashref (id/query fields; OutputStatusLoadMatch
...
# Returns the loaded OutputStatus data (hashref); dies with BluefinTecsMerchantPortalError
# on failure.
sub load {
