# BluefinTecsMerchantPortal SDK OutputList entity
...
sub make {
  ...
  return OutputListEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputList data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputList fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputList.
#
# reqdata: body data hashref (OutputListCreateData shape). ctrl: optional
# per-call control. Returns the created OutputList data (hashref); dies
