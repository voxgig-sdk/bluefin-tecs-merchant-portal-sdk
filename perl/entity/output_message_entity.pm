# BluefinTecsMerchantPortal SDK OutputMessage entity
...
sub make {
  ...
  return OutputMessageEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputMessage data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputMessage fields).
sub match_get {
  my ($self) = @_;
...
# Load a single OutputMessage.
#
# reqmatch: match criteria hashref (id/query fields; OutputMessageLoadMatch
...
# Returns the loaded OutputMessage data (hashref); dies with BluefinTecsMerchantPortalError
# on failure.
sub load {
