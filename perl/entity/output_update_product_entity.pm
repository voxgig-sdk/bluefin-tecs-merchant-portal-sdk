# BluefinTecsMerchantPortal SDK OutputUpdateProduct entity
...
package OutputUpdateProductEntity;
...
sub make {
  ...
  return OutputUpdateProductEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputUpdateProduct data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputUpdateProduct fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputUpdateProduct.
#
# reqdata: body data hashref (OutputUpdateProductCreateData shape). ctrl: optional
# per-call control. Returns the created OutputUpdateProduct data (hashref); dies
