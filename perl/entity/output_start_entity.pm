# BluefinTecsMerchantPortal SDK OutputStart entity
...
sub make {
  ...
  return OutputStartEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputStart data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputStart fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputStart.
#
# reqdata: body data hashref (OutputStartCreateData shape). ctrl: optional
# per-call control. Returns the created OutputStart data (hashref); dies
