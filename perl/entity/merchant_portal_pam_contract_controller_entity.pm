...
package MerchantPortalPamContractControllerEntity;
...
sub new {
  ...
  elsif (BluefinTecsMerchantPortalHelpers::is_false($entopts->{active})) {
    # keep false
  }
  else {
    $entopts->{active} = Voxgig::Struct::JTRUE();
  ...
  my $self = bless {
    _name => 'merchant_portal_pam_contract_controller',
    _client => $client,
...
sub data_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_data} = BluefinTecsMerchantPortalHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetData');
  }
  return;
}

# Returns the current MerchantPortalPamContractController data (hashref).
sub data_get {
  my ($self) = @_;
...
sub match_set {
  my ($self, $args) = @_;
  if ($args) {
    $self->{_match} = BluefinTecsMerchantPortalHelpers::to_map(Voxgig::Struct::clone($args)) || {};
    $self->{_utility}{feature_hook}->($self->{_entctx}, 'SetMatch');
  }
  return;
}

# Returns the current match filter (any subset of MerchantPortalPamContractController fields).
sub match_get {
  my ($self) = @_;
...
#
# reqdata: body data hashref (MerchantPortalPamContractControllerCreateData shape). ctrl: optional
# per-call control. Returns the created MerchantPortalPamContractController data (hashref); dies
# with BluefinTecsMerchantPortalError on failure.
sub create {
  ...
  return $self->_run_op($ctx, sub {
    ...
    if ($result) {
      if ($result->{resdata}) {
        $self->{_data} = BluefinTecsMerchantPortalHelpers::to_map(
...
sub stream {
  ...
  my $utility = $self->{_utility};
  $callopts = BluefinTecsMerchantPortalHelpers::to_map($callopts) || {};
  my $signal = BluefinTecsMerchantPortalHelpers::gp($callopts, 'signal');
  my $ctrl = BluefinTecsMerchantPortalHelpers::to_map(
    BluefinTecsMerchantPortalHelpers::gp($callopts, 'ctrl')) || {};
  ...
  my $ctx = $utility->{make_context}->({
    ...
    'match' => $self->{_match},
    'data' => $self->{_data},
    %{ BluefinTecsMerchantPortalHelpers::to_map($args) || {} },
  }, $self->{_entctx});

  # Outbound: expose an async-iterable/list payload so the request builder /
  # transport can stream it as the request body.
  my $body = BluefinTecsMerchantPortalHelpers::gp($callopts, 'body');
  if (defined $body) {
    my $reqdata = BluefinTecsMerchantPortalHelpers::to_map($ctx->{reqdata}) || {};
    $reqdata->{'body$'} = $body;
    $ctx->{reqdata} = $reqdata;
    $ctx->{stream_out} = $body;
  }
