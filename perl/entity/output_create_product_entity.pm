# BluefinTecsMerchantPortal SDK OutputCreateProduct entity
...
package OutputCreateProductEntity;
...
sub make {
  ...
  return OutputCreateProductEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputCreateProduct data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputCreateProduct fields).
sub match_get {
  my ($self) = @_;
...
# Create a new OutputCreateProduct.
#
# reqdata: body data hashref (OutputCreateProductCreateData shape). ctrl: optional
# per-call control. Returns the created OutputCreateProduct data (hashref); dies
