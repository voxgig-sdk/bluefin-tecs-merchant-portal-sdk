# BluefinTecsMerchantPortal SDK OutputDetail entity
...
sub make {
  ...
  return OutputDetailEntity->new($self->{_client}, $opts);
}
...
# Returns the current OutputDetail data (hashref).
sub data_get {
  my ($self) = @_;
...
# Returns the current match filter (any subset of OutputDetail fields).
sub match_get {
  my ($self) = @_;
...
# Load a single OutputDetail.
#
# reqmatch: match criteria hashref (id/query fields; OutputDetailLoadMatch
...
# Returns the loaded OutputDetail data (hashref); dies with BluefinTecsMerchantPortalError
# on failure.
sub load {
