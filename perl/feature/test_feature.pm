# BluefinTecsMerchantPortal SDK test feature
...
package BluefinTecsMerchantPortalTestFeature;
...
sub init {
  ...
  my $entity = BluefinTecsMerchantPortalHelpers::gp($options, 'entity');
  ...
  my $envelope = sub {
    ...
    my $tm = BluefinTecsMerchantPortalHelpers::gp($fctx->{point}, 'transform');
    my $spec = BluefinTecsMerchantPortalHelpers::gp($tm, 'res');
  ...
  my $test_fetcher = sub {
    ...
    my $op = $fctx->{op};
    my $entmap = BluefinTecsMerchantPortalHelpers::gp($entity, $op->{entity});
    ...
    my $resolve_match = sub {
      ...
      for my $src ($fctx->{match}, $fctx->{data}) {
        ...
        my $v = BluefinTecsMerchantPortalHelpers::gp($src, 'id');
    ...
    if ('load' eq $op->{name}) {
      ...
      my $ent = BluefinTecsMerchantPortalHelpers::ge($found, 0);
      return $respond->($fctx, 404, undef, { 'statusText' => 'Not found' })
        unless BluefinTecsMerchantPortalHelpers::rb_truthy($ent);
      ...
      if (Voxgig::Struct::ismap($fctx->{reqdata})) {
        ...
        if ($op->{alias}) {
          my $alias_id = BluefinTecsMerchantPortalHelpers::gp($op->{alias}, 'id');
      ...
      if (Voxgig::Struct::ismap($ent)) {
        my $id = BluefinTecsMerchantPortalHelpers::gp($ent, 'id');
  ...
  my $net = BluefinTecsMerchantPortalHelpers::gp($options, 'net');
...
sub make_netsim {
  ...
  my $do_sleep = sub {
    ...
    if (ref $net->{sleep} eq 'CODE') {
      ...
      BluefinTecsMerchantPortalHelpers::sleep_ms($ms);
    }
  };
  ...
  return sub {
    ...
    if (BluefinTecsMerchantPortalHelpers::is_true($net->{offline})) {
...
sub build_args {
  ...
  my $points = BluefinTecsMerchantPortalHelpers::gpath($ctx->{config},
    "entity.$entname.op.$opname.points");
  my $point = BluefinTecsMerchantPortalHelpers::ge($points, -1);

  my $params_path = BluefinTecsMerchantPortalHelpers::gpath($point, 'args.params');
    ...
    if ($keys) {
      for my $key (@$keys) {
        ...
        if ($is_id || $is_reqd) {
          ...
          my $ka = $op->{alias} ? BluefinTecsMerchantPortalHelpers::gp($op->{alias}, $key) : undef;
