...
package BluefinTecsMerchantPortalContext;
...
sub new {
  ...
  my $gcp = \&BluefinTecsMerchantPortalHelpers::get_ctx_prop;
  ...
  $self->{ctrl} = BluefinTecsMerchantPortalControl->new();
  ...
  $self->{data} = BluefinTecsMerchantPortalHelpers::to_map($gcp->($ctxmap, 'data')) || {};
  $self->{reqdata} = BluefinTecsMerchantPortalHelpers::to_map($gcp->($ctxmap, 'reqdata')) || {};
  $self->{match} = BluefinTecsMerchantPortalHelpers::to_map($gcp->($ctxmap, 'match')) || {};
  $self->{reqmatch} = BluefinTecsMerchantPortalHelpers::to_map($gcp->($ctxmap, 'reqmatch')) || {};

  my $pt = $gcp->($ctxmap, 'point');
  $self->{point} = Voxgig::Struct::ismap($pt) ? $pt : ($basectx ? $basectx->{point} : undef);

  my $sp = $gcp->($ctxmap, 'spec');
  $self->{spec} = (Scalar::Util::blessed($sp) && $sp->isa('BluefinTecsMerchantPortalSpec'))
    ? $sp : ($basectx ? $basectx->{spec} : undef);

  my $r = $gcp->($ctxmap, 'result');
  $self->{result} = (Scalar::Util::blessed($r) && $r->isa('BluefinTecsMerchantPortalResult'))
    ? $r : ($basectx ? $basectx->{result} : undef);

  my $rp = $gcp->($ctxmap, 'response');
  $self->{response} = (Scalar::Util::blessed($rp) && $rp->isa('BluefinTecsMerchantPortalResponse'))
...
sub resolve_op {
  ...
  return BluefinTecsMerchantPortalOperation->new({}) if '' eq $opname;

  my $opcfg = BluefinTecsMerchantPortalHelpers::gpath($self->{config}, "entity.$entname.op.$opname");

  my $input = ($opname eq 'update' || $opname eq 'create') ? 'data' : 'match';

  my $points = [];
  if (Voxgig::Struct::ismap($opcfg)) {
    my $t = BluefinTecsMerchantPortalHelpers::gp($opcfg, 'points');
    $points = $t if Voxgig::Struct::islist($t);
  }

  my $op = BluefinTecsMerchantPortalOperation->new({
    entity => $entname,
    name   => $opname,
    input  => $input,
    points => $points,
...
sub make_error {
  my ($self, $code, $msg) = @_;
  return BluefinTecsMerchantPortalError->new($code, $msg, $self);
}
