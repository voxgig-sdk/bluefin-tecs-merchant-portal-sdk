#!perl
# BluefinTecsMerchantPortal SDK primary utility test
#
...
#   live blessed context with BluefinTecsMerchantPortalOmni::livectx before calling a
...
my $_runner = BluefinTecsMerchantPortalOmni::make_runner($TEST_JSON, BluefinTecsMerchantPortalSDK->test(undef, undef));
...

{
  ...
  our @ISA = ('BluefinTecsMerchantPortalBaseFeature');

  sub new {
    my ($class, $fn) = @_;
    my $self = BluefinTecsMerchantPortalBaseFeature::new($class);
    $self->{hook_fn} = $fn;
    return $self;
  }
...
sub _err_from_map {
  ...
  return BluefinTecsMerchantPortalError->new($code, $msg);
}
...
sub _def_setup {
  ...
  return BluefinTecsMerchantPortalOmni::tostruct(Voxgig::Omni::Util::clone($setup));
}
...
runsection('done', sub {
  my ($view) = @_;
  return $utility->{done}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('makeError', sub {
  my ($view, $errmap) = @_;
  my $ctx = BluefinTecsMerchantPortalOmni::livectx($view);
...

{
  ...
  $ctx->{result} = BluefinTecsMerchantPortalResult->new({
    'ok' => 0,
    'resdata' => { 'id' => 'safe01' },
  });
...

{
  ...
  my $feature = BluefinTecsMerchantPortalBaseFeature->new;
...
{
  my $hook_client = BluefinTecsMerchantPortalSDK->test(undef, undef);
...
{
  my $init_client = BluefinTecsMerchantPortalSDK->test(undef, undef);
...
{
  my $calls = [];
  my $live_client = BluefinTecsMerchantPortalSDK->new({
...
{
  my $blocked_client = BluefinTecsMerchantPortalSDK->new({
    'base' => 'http://localhost:8080',
    'system' => {
...
runsection('makeResponse', sub {
  my ($view) = @_;
  return _unwrap($utility->{make_response}->(BluefinTecsMerchantPortalOmni::livectx($view)));
});
...
{
  my $ctx = make_test_full_ctx($client, $utility);
  $ctx->{spec} = BluefinTecsMerchantPortalSpec->new({ 'step' => 'start' });
  $ctx->{result} = undef;
  my (undef, $err) = $utility->{make_result}->($ctx);
...
{
  my $spec_client = BluefinTecsMerchantPortalSDK->test(undef, _def_setup('makeSpec'));
...
runsection('makePoint', sub {
  my ($view) = @_;
  my ($point, $err) = $utility->{make_point}->(BluefinTecsMerchantPortalOmni::livectx($view));
  # The corpus asserts refusals by code (`match: {out: {code: ...}}`), so
  # an error is the RESULT here, flattened to the map the corpus reads.
  return BluefinTecsMerchantPortalOmni::errify($err) if defined $err;
  return $point;
});
...
runsection('makeUrl', sub {
  my ($view) = @_;
  my $ctx = BluefinTecsMerchantPortalOmni::livectx($view);
  $ctx->{result} = BluefinTecsMerchantPortalResult->new({}) unless $ctx->{result};
...
runsection('operator', sub {
  my ($vin) = @_;
  $vin = {} unless ref($vin) eq 'HASH';
  my $op = BluefinTecsMerchantPortalOperation->new($vin);
  return {
    'entity' => $op->{entity},
    'name' => $op->{name},
    'input' => $op->{input},
...
runsection('param', sub {
  my ($view, $paramdef) = @_;
  return $utility->{param}->(BluefinTecsMerchantPortalOmni::livectx($view), $paramdef);
});
...
{
  my $auth_client = BluefinTecsMerchantPortalSDK->test(undef, _def_setup('prepareAuth'));
...
runsection('prepareBody', sub {
  my ($view) = @_;
  return $utility->{prepare_body}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('prepareHeaders', sub {
  my ($view) = @_;
  return $utility->{prepare_headers}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('prepareMethod', sub {
  my ($view) = @_;
  return $utility->{prepare_method}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('prepareParams', sub {
  my ($view) = @_;
  return $utility->{prepare_params}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('preparePath', sub {
  my ($view) = @_;
  return $utility->{prepare_path}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('prepareQuery', sub {
  my ($view) = @_;
  return $utility->{prepare_query}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('resultBasic', sub {
  my ($view) = @_;
  my $result = $utility->{result_basic}->(BluefinTecsMerchantPortalOmni::livectx($view));
...
runsection('resultBody', sub {
  my ($view) = @_;
  return $utility->{result_body}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('resultHeaders', sub {
  my ($view) = @_;
  return $utility->{result_headers}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('transformRequest', sub {
  my ($view) = @_;
  return $utility->{transform_request}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
...
runsection('transformResponse', sub {
  my ($view) = @_;
  return $utility->{transform_response}->(BluefinTecsMerchantPortalOmni::livectx($view));
});
