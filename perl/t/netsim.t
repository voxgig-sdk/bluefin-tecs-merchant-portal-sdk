...
{
  my $sdk = BluefinTecsMerchantPortalSDK->test({ 'net' => { 'offline' => 1 } }, undef);
...
{
  my $sdk = BluefinTecsMerchantPortalSDK->test({ 'net' => { 'failTimes' => 1, 'failStatus' => 503 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $sdk = BluefinTecsMerchantPortalSDK->test({ 'net' => { 'errorTimes' => 1 } }, undef);
  my $res = $sdk->direct({ 'path' => '/ping' });
...
{
  my $delay = 60;
  my $sdk = BluefinTecsMerchantPortalSDK->test({ 'net' => { 'latency' => $delay } }, undef);
...
{
  my $sdk = BluefinTecsMerchantPortalSDK->test(undef, undef);
