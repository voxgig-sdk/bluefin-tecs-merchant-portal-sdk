...
sub new {
  ...
  my $config = BluefinTecsMerchantPortalConfig::shared_config();
...
sub _op_allowed {
  my ($self, $op) = @_;
  my $allow = BluefinTecsMerchantPortalHelpers::gpath($self->{options}, 'allow.op');
...
sub graphql {
  ...
  my $errors = BluefinTecsMerchantPortalHelpers::gpath($res, 'data.errors');
