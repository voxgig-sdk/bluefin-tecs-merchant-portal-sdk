// OutputUpdateProduct entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputUpdateProductEntity = struct {
    name: []const u8 = "output_update_product",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantPortalSDK, entopts_in: Value) *OutputUpdateProductEntity {
        ...
        const e = h.A().create(OutputUpdateProductEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputUpdateProductEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputUpdateProductEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputUpdateProductEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputUpdateProductEntity, ctx: *Context, post_done: *const fn (*OutputUpdateProductEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputUpdateProductEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputUpdateProductEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputUpdateProductEntity, ctx: *Context, post_done: *const fn (*OutputUpdateProductEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputUpdateProductEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputUpdateProductEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputUpdateProductEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputUpdateProductEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputUpdateProductEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputUpdateProductEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputUpdateProductEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputUpdateProductEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputUpdateProductEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputUpdateProductEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputUpdateProductEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputUpdateProductEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
