// OutputMoveTid entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputMoveTidEntity = struct {
    name: []const u8 = "output_move_tid",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantPortalSDK, entopts_in: Value) *OutputMoveTidEntity {
        ...
        const e = h.A().create(OutputMoveTidEntity) catch unreachable;
        e.* = .{
    ...
    fn ent_ctx(self: *OutputMoveTidEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputMoveTidEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputMoveTidEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputMoveTidEntity, ctx: *Context, post_done: *const fn (*OutputMoveTidEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputMoveTidEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputMoveTidEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputMoveTidEntity, ctx: *Context, post_done: *const fn (*OutputMoveTidEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputMoveTidEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputMoveTidEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputMoveTidEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputMoveTidEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputMoveTidEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputMoveTidEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputMoveTidEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputMoveTidEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputMoveTidEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputMoveTidEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputMoveTidEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputMoveTidEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
