// OutputStart entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputStartEntity = struct {
    name: []const u8 = "output_start",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantPortalSDK, entopts_in: Value) *OutputStartEntity {
        ...
        const e = h.A().create(OutputStartEntity) catch unreachable;
        e.* = .{
            .name = "output_start",
            .client = client,
    ...
    fn ent_ctx(self: *OutputStartEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputStartEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputStartEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputStartEntity, ctx: *Context, post_done: *const fn (*OutputStartEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputStartEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputStartEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputStartEntity, ctx: *Context, post_done: *const fn (*OutputStartEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputStartEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputStartEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputStartEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputStartEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputStartEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputStartEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputStartEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("load", self.name) };
    }

    pub fn list(self: *OutputStartEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
    ...
    pub fn create(self: *OutputStartEntity, reqdata: Value, ctrl: Value) EntResult {
    ...
    fn create_post_done(self: *OutputStartEntity, ctx: *Context) void {
    ...
    pub fn update(self: *OutputStartEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputStartEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
