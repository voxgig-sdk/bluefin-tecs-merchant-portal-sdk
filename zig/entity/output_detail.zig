// OutputDetail entity client (generated — mirrors the go/rust Entity fragment).
...
pub const OutputDetailEntity = struct {
    name: []const u8 = "output_detail",
    ...
    pub fn new(client: *sdk.BluefinTecsMerchantPortalSDK, entopts_in: Value) *OutputDetailEntity {
        ...
        const e = h.A().create(OutputDetailEntity) catch unreachable;
        e.* = .{
            .name = "output_detail",
            .client = client,
    ...
    fn ent_ctx(self: *OutputDetailEntity) *Context {
        return self.entctx orelse unreachable;
    }

    fn opError(self: *OutputDetailEntity, ctx: *Context) OpResult {
        const v = self.utility.make_error(ctx) catch return .{ .err = ctx.pending_err.? };
        return .{ .ok = v };
    }

    fn doneResult(self: *OutputDetailEntity, ctx: *Context) OpResult {
    ...
    fn run_op_ent(self: *OutputDetailEntity, ctx: *Context, post_done: *const fn (*OutputDetailEntity, *Context) void) EntResult {
    ...
    // live record.
    pub fn mark_deleted(self: *OutputDetailEntity) void {
        self.deleted = true;
    }

    pub fn is_deleted(self: *OutputDetailEntity) bool {
        return self.deleted;
    }

    fn run_op(self: *OutputDetailEntity, ctx: *Context, post_done: *const fn (*OutputDetailEntity, *Context) void) OpResult {
    ...
    pub fn stream(self: *OutputDetailEntity, action: []const u8, args: Value, callopts: Value) []Value {
        const utility = self.utility;
    ...
    fn e_of(p: *anyopaque) *OutputDetailEntity {
    ...
    fn v_make(p: *anyopaque) Entity {
        ...
        return OutputDetailEntity.new(self.client, opts).asEntity();
    }
    ...
    pub fn asEntity(self: *OutputDetailEntity) Entity {
        return .{ .ptr = @ptrCast(self), .vtable = &entity_vtable };
    }

    fn data_impl(self: *OutputDetailEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
    ...
    fn matchv_impl(self: *OutputDetailEntity, args: ?Value) Value {
        if (args) |arg| {
            if (!h.is_noval(arg)) {
                const cloned = h.to_map(h.clone(arg));
    ...
    pub fn load(self: *OutputDetailEntity, reqmatch: Value, ctrl: Value) EntResult {
    ...
    fn load_post_done(self: *OutputDetailEntity, ctx: *Context) void {
    ...
    pub fn list(self: *OutputDetailEntity, _reqmatch: Value, _ctrl: Value) EntListResult {
        _ = _reqmatch;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("list", self.name) };
    }

    pub fn create(self: *OutputDetailEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("create", self.name) };
    }

    pub fn update(self: *OutputDetailEntity, _reqdata: Value, _ctrl: Value) EntResult {
        _ = _reqdata;
        _ = _ctrl;
        return .{ .err = h.unsupported_op("update", self.name) };
    }

    pub fn remove(self: *OutputDetailEntity, _reqmatch: Value, _ctrl: Value) EntResult {
        _ = _reqmatch;
        _ = _ctrl;
